(function () {
  'use strict';
  const backgroundIcon = `<svg viewBox="0 0 24 24"><path d="M3 3h18v18H3V3Zm2 2v14h14V5H5Zm1 11 3.4-4.2 2.4 2.8 1.8-2.1L18 17H6v-1Zm8.5-8.5a2 2 0 1 1 0 4 2 2 0 0 1 0-4Z"/></svg>`;
  const addImageIcon = `<svg viewBox="0 0 24 24"><path d="M4 3h11v2H5v14h14v-7h2v9H3V3h1Zm3 13 3.2-4 2.4 2.9 1.9-2.2L18 16.5V17H7v-1Zm7-9h3V4h2v3h3v2h-3v3h-2V9h-3V7ZM9 7a2 2 0 1 1 0 4 2 2 0 0 1 0-4Z"/></svg>`;
  const trimIcon = `<svg viewBox="0 0 24 24"><path d="M6 2h2v4h8V2h2v4h4v2h-4v8h4v2h-4v4h-2v-4H6V8H2V6h4V2Zm2 6v8h8V8H8Z"/></svg>`;
  const photoLayoutIcon = `<svg viewBox="0 0 24 24"><path d="M3 3h8v8H3V3Zm2 2v4h4V5H5Zm8-2h8v8h-8V3Zm2 2v4h4V5h-4ZM3 13h8v8H3v-8Zm2 2v4h4v-4H5Zm8-2h8v8h-8v-8Zm2 2v4h4v-4h-4Z"/></svg>`;
  const exportTemplateIcon = `<svg viewBox="0 0 24 24"><path d="M5 3h10l4 4v4h-2V8h-3V5H5v14h5v2H3V3h2Zm8 9h2v5.6l1.8-1.8 1.4 1.4-4.2 4.2-4.2-4.2 1.4-1.4 1.8 1.8V12Z"/></svg>`;
  const createTools = [
    {
      name: 'shapes',
      title: window.lang === 'ru' ? 'Фигуры' : 'Shapes',
      icon: `<svg viewBox="0 0 24 24"><rect x="2" y="3" width="9" height="9" rx="1"></rect><circle cx="17" cy="17" r="5"></circle></svg>`,
    },
    {
      name: 'draw',
      title: window.lang === 'ru' ? 'Кисть' : 'Free draw',
      icon: `<svg viewBox="0 0 24 24"><path d="M3 17c4-8 7-12 10-12 2 0 2 3 0 6-2 4-1 6 2 6 2 0 4-1 6-3v4c-2 2-5 3-8 3-5 0-6-4-4-8-1 1-2 3-3 6z"></path></svg>`,
    },
    {
      name: 'line',
      title: window.lang === 'ru' ? 'Линия' : 'Line',
      icon: `<svg viewBox="0 0 24 24"><path d="M4 18.6 18.6 4 20 5.4 5.4 20z"></path><circle cx="4" cy="20" r="2"></circle><circle cx="20" cy="4" r="2"></circle></svg>`,
    },
    {
      name: 'path',
      title: window.lang === 'ru' ? 'Сплайн' : 'Spline',
      icon: `<svg viewBox="0 0 24 24"><path d="M4 18c5 0 3-12 9-12h5v3h-5c-3 0-2 12-9 12z"></path><circle cx="4" cy="19" r="3"></circle><circle cx="19" cy="7" r="3"></circle></svg>`,
    },
  ];
  ...
  var defaultButtons = [
    ...
    {
      name: 'text',
      title: window.lang === 'ru' ? 'Добавить текст' : 'Add text',
      label: window.lang === 'ru' ? 'Текст' : 'Text',
    ...
    {
      name: 'photo-layout',
      title: window.lang === 'ru' ? 'Расположение фотографий' : 'Photo layout',
      label: window.lang === 'ru' ? 'Фото' : 'Photos',
      icon: photoLayoutIcon
    },
    {
      name: 'trim',
      title: window.lang === 'ru' ? 'Показать обрезку печати' : 'Show print trim',
      label: window.lang === 'ru' ? 'Обрезка' : 'Trim',
      icon: trimIcon
    },
    {
      name: 'background',
      title: window.lang === 'ru' ? 'Фон макета' : 'Canvas background',
      label: window.lang === 'ru' ? 'Фон' : 'Background',
      icon: backgroundIcon
    },
    // {
    //   name: 'background',
    ...
    {
      name: 'upload-image',
      title: window.lang === 'ru' ? 'Добавить картинку как объект' : 'Add image as an object',
      label: window.lang === 'ru' ? 'Добавить картинку' : 'Add image',
      icon: addImageIcon
    },
    {
      name: 'export-template',
      title: window.lang == 'ru' ? 'Импорт и экспорт шаблона' : 'Import and export template',
      label: window.lang == 'ru' ? 'Шаблон' : 'Template',
      icon: exportTemplateIcon
    },
    {
      name: 'create-tools',
      title: window.lang === 'ru' ? 'Фигуры и рисование' : 'Shapes and drawing',
  ...
  const defaultExtendedButtons = [
    ...
    },
    {
      ...
      icon: `<svg viewBox="0 0 16 16"><path d="m12 8-8 5V3z"></path></svg>`
    },
    // {
    //   name: 'animation',
    //   title: 'Play animation',
    //   icon: `<svg viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" >  <path d="m11.596 8.697-6.363 3.692c-.54.313-1.233-.066-1.233-.697V4.308c0-.63.692-1.01 1.233-.696l6.363 3.692a.802.802 0 0 1 0 1.393"/></svg>`
    // },
    {
      name: 'undo',
      title: 'Undo',
    ...
    // },
    {
      name: 'fullscreen',
      title: 'Fullscreen',
      icon: `<svg viewBox="0 0 24 24"><path d="M3 3h8v3H6v5H3zm10 0h8v8h-3V6h-5zM3 13h3v5h5v3H3zm15 0h3v8h-8v-3h5z"></path></svg>`
    },
    // {
    //   name: 'fullscreen',
    //   title: 'Fullscreen',
    ...
    try {
      this.containerEl.append(`
        <div id="toolbar" class="toolbar">
          <div class="studio-brand">
            <span class="studio-brand-mark">TS</span>
            <span class="studio-brand-copy">
              <strong>Template Studio</strong>
              <small>Photobooth layouts</small>
            </span>
          </div>
          <div class="layout-tabs" role="tablist" aria-label="${window.lang === 'ru' ? 'Макет шаблона' : 'Template layout'}">
            <button class="active" type="button" data-layout-tab="grid" role="tab" aria-selected="true">
              <span>${window.lang === 'ru' ? 'Грид' : 'Grid'}</span>
              <i data-layout-status="grid" aria-hidden="true"></i>
            </button>
            <button type="button" data-layout-tab="strips" role="tab" aria-selected="false">
              <span>${window.lang === 'ru' ? 'Стрипсы' : 'Strips'}</span>
              <i data-layout-status="strips" aria-hidden="true"></i>
            </button>
          </div>
        ...
        <div id="footerbar" class="toolbar">
          <div class="footer-actions">
          </div>
          <div class="canvas-meta">
            <span id="canvas-layout-name">${window.lang === 'ru' ? 'Грид' : 'Grid'}</span>
            <strong id="canvas-layout-size">${this.dimensions.width} × ${this.dimensions.height}</strong>
          </div>
      ...
      buttons.forEach(item => {
        $(`${this.containerSelector} #toolbar .main-buttons`).append(
          `<button type="button" id="${item.name}" class="${item.label ? 'toolbar-button-labeled' : ''}" title="${item.title}" aria-label="${item.title}">${item.icon}${item.label ? `<span class="toolbar-button-label">${item.label}</span>` : ''}</button>`
        );
      })

      const createToolsButton = document.querySelector(
        `${this.containerSelector} #create-tools`,
      );
      let createToolsMenu = null;

      if (createToolsButton) {
        document.querySelector(`${this.containerSelector} #toolbar`).insertAdjacentHTML('beforeend', `
          <div class="toolbar-tool-popover" id="create-tools-menu" role="menu" hidden>
            ${createTools.map(tool => `
              <button type="button" data-create-tool="${tool.name}" role="menuitem">
                ${tool.icon}<span>${tool.title}</span>
              </button>
            `).join('')}
          </div>
        `);
        createToolsMenu = document.querySelector(
          `${this.containerSelector} #create-tools-menu`,
        );
      }

      const closeCreateToolsMenu = () => {
        if (createToolsMenu) {
          createToolsMenu.hidden = true;
          createToolsButton.setAttribute('aria-expanded', 'false');
        }
      };

      const toggleCreateToolsMenu = () => {
        if (!createToolsMenu) return;
        const willOpen = createToolsMenu.hidden;
        closeCreateToolsMenu();
        if (!willOpen) return;

        createToolsMenu.hidden = false;
        const buttonBounds = createToolsButton.getBoundingClientRect();
        const menuBounds = createToolsMenu.getBoundingClientRect();
        createToolsMenu.style.left = `${Math.max(8, Math.min(
          buttonBounds.left,
          window.innerWidth - menuBounds.width - 8,
        ))}px`;
        createToolsMenu.style.top = `${buttonBounds.bottom + 8}px`;
        createToolsButton.setAttribute('aria-expanded', 'true');
      };

      createToolsButton?.setAttribute('aria-haspopup', 'menu');
      createToolsButton?.setAttribute('aria-expanded', 'false');

      extendedButtons.forEach(item => {
        $(`${this.containerSelector} #footerbar .footer-actions`).append(
          `<button type="button" id="${item.name}" title="${item.title}" aria-label="${item.title}">${item.icon}</button>`
        );
      })

      $(document).on('click', '.toolbar button', async function (e) {
        if (this.matches('[data-layout-tab]')) {
          return;
        }

        const createTool = this.dataset.createTool;
        if (createTool) {
          closeCreateToolsMenu();
          _self.setActiveTool(createTool, true);
          return;
        }
        ...
        if (id === 'text') {
          ...
          const text = new fabric.IText(window.lang == 'ru' ? 'Введите текст' : 'Enter text', {
            left: _self.canvas.originalW / 2,
            top: _self.canvas.originalH / 2,
            fontSize: 43,
          ...
          _self.canvas.setActiveObject(text);
          _self.canvas.renderAll();

          return;
        }

        else if (id === 'upload-image') {
          ...
          if (typeof _self.clearProject === 'function') {
            void _self.clearProject();
            return;
          }

          if (!window.confirm(
            window.lang === 'ru'
              ? 'Очистить весь проект? Это действие нельзя отменить.'
              : 'Clear the entire project? This cannot be undone.',
          )) {
            return;
          }

          _self.canvas.historyProcessing = true;
          _self.canvas.discardActiveObject();
          _self.canvas.getObjects().forEach(obj => _self.canvas.remove(obj));
          _self.canvas.backgroundImage = null;
          _self.canvas.overlayImage = null;
          _self.canvas.setBackgroundColor('#ffffff');
          _self.canvas.renderAll();
          _self.canvas.historyProcessing = false;
          _self.canvas.clearHistory();
          _self.canvas._historySaveAction();

          return;
        }

        else if (id === 'download-image') { // as png
          let canvasCopy = _self.canvas;
          const currentZoom = canvasCopy.getZoom();
          const previewObjects = canvasCopy.getObjects()
            .filter(object => (
              object.excludeFromExport || object.kind === 'photo-slot'
            ));
          const visibility = previewObjects.map(object => object.visible);
          let imageData;

          previewObjects.forEach(object => object.set('visible', false));
          canvasCopy.setZoom(1);

          try {
            canvasCopy.requestRenderAll();
            imageData = canvasCopy.toDataURL({
              format: 'png',
              multiplier: 1,
              width: canvasCopy.originalW,
              height: canvasCopy.originalH
            });
          } finally {
            previewObjects.forEach((object, index) => {
              object.set('visible', visibility[index]);
            });
            canvasCopy.setZoom(currentZoom);
            canvasCopy.requestRenderAll();
          }

          _self.downloadImage(imageData);

          return;
        }

        else if (id === 'animation') {
          _self.animations.runAnimation();
          ...
          $(_self.containerSelector).append(`<div class="custom-modal-container">
              <div class="custom-modal-content">
                <div class="button-download" type="svg">Download as SVG</div>
                <div class="button-download" type="png">Download as PNG</div>
                <div class="button-download" type="jpg">Download as JPG</div>
                <div class="button-download" type="webp">Download as WEBP</div>
                <div class="button-download" type="json">Download as JSON</div>
              </div>
            </div>`)
          ...
          if (_self.canvas.canUndo && _self.canvas.canUndo()) {
            _self.canvas.undo(() => {
              _self.canvas.renderAll();
          ...
          if (_self.canvas.canRedo && _self.canvas.canRedo()) {
            _self.canvas.redo(() => {
              _self.canvas.renderAll();
              console.log('Повтор выполнен');
            });
          }
        ...
        else if (id === 'photo-layout') {
          const panel = document.querySelector(
            `${_self.containerSelector} #photo-layout-panel`,
          );

          if (!panel?.classList.contains('visible')) {
            _self.ensurePhotoLayout?.();
          }
        }

        _self.setActiveTool(id, true);
      })

      document.addEventListener('click', event => {
        if (
          createToolsMenu
          && !createToolsMenu.hidden
          && !event.target.closest('#create-tools, #create-tools-menu')
        ) {
          closeCreateToolsMenu();
        }
      });
      ...
      window.addEventListener('resize', closeCreateToolsMenu);
