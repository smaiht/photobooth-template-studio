  ...
  var selectionSettings = function () {
    ...
    (() => {
          ...
          <div class="sizes">
            ...
            <div class="input-container">
              ...
              <div class="custom-number-input">
                ...
                <input type="number" min="4" max="2000" value="20" id="fontSize">
            ...
            <div class="input-container">
              ...
              <div class="custom-number-input">
                <button class="decrease">-</button>
                <input type="number" min="1" max="1000" step="100" value="400" id="fontWeight">
                <button class="increase">+</button>
              </div>
            </div>
            ...
            <div class="input-container">
              <label>
                ${window.lang == 'ru' ? 'Ширина области текста' : 'Text Box Width'}
              </label>
              <div class="custom-number-input">
                <button class="decrease">-</button>
                <input type="number" min="50" max="10000" step="10" value="1000" id="textBoxWidth">
                <button class="increase">+</button>
              </div>
            </div>
            <p class="text-box-width-hint">
              ${window.lang == 'ru'
                ? 'Меняет перенос строк без масштабирования. Высота подстраивается автоматически.'
                : 'Changes wrapping without scaling. Height grows automatically.'}
            </p>
            ...
            <div class="input-container">
              ...
              <div class="custom-number-input">
              <button class="decrease">-</button>
              <input type="number" min="0.5" max="4" value="1.2" step="0.1" id="lineHeight">
              <button class="increase">+</button>
              </div>
            </div>
            ...
            <div class="input-container">
              ...
              <div class="custom-number-input">
              <button class="decrease">-</button>
              <input type="number" min="-2000" max="2000" step="100" value="0" id="charSpacing">
              <button class="increase">+</button>
              </div>
            </div>
            ...
            <div class="input-container">
              ...
              <div class="custom-number-input">
                <button class="decrease">-</button>
                <input type="number" min="0" max="500" value="0" id="textStrokeWidth">
                <button class="increase">+</button>
              </div>
            </div>
          ...
          <div class="align">
            ...
            <select id="text-align">
              ...
              <option value="justify">
                ${window.lang == 'ru' ? 'Растянуть' : 'Justify'}
              </option>
            </select>
            </div>
          </div>
      ...
      const ensureTextbox = object => {
        ...
        options.styles = fabric.util.object.clone(object.styles || {}, true);
        ...
        const textbox = new fabric.Textbox(object.text, {
          ...
          originX: 'center',
          originY: 'center',
          width: Math.max(50, object.width || 1000),
          paintFirst: 'stroke',
        });
        const wasProcessingHistory = canvas.historyProcessing;
        ...
        try {
          ...
          canvas.historyProcessing = wasProcessingHistory;
        }
        canvas.setActiveObject(textbox);
        _self.activeSelection = textbox;

        const panel = document.querySelector(
          `${_self.containerSelector} .toolpanel#select-panel`,
        );
        panel.classList.remove('type-i-text', 'type-text');
        panel.classList.add('type-textbox');
        return textbox;
      };
        ...
        switch (type) {
          ...
          case 'subscript':
            _self.setActiveFontStyle(
              _self.activeSelection,
              'deltaY',
              _self.getActiveFontStyle(_self.activeSelection, 'deltaY') > 0
                ? 0
                : Math.round((_self.activeSelection.fontSize || 40) * 0.3),
            )
            break;
          case 'superscript':
            _self.setActiveFontStyle(
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .sizes #fontSize, ${this.containerSelector} .toolpanel#select-panel .sizes #fontWeight`).change(function () {
        const value = parseFloat($(this).val());
        const type = $(this).attr('id');

        if (!Number.isFinite(value) || !_self.activeSelection) return;
        _self.setActiveFontStyle(_self.activeSelection, type, value);
        _self.canvas.renderAll();
        _self.canvas.fire('object:property-realy-changed');
      });

      $(`${this.containerSelector} .toolpanel#select-panel #textBoxWidth`).change(function () {
        const width = Number($(this).val());
        const object = ensureTextbox(_self.activeSelection);

        if (!object || !Number.isFinite(width) || width < 50) {
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .sizes #lineHeight, ${this.containerSelector} .toolpanel#select-panel .sizes #charSpacing`).change(function () {
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .family #font-family`).change(function() {
        ...
        if (fontAsset) {
          _self.setActiveFontStyle(object, 'fontFamily', fontFamily);
          object.set('fontFile', fontAsset.fileName);
          _self.canvas.renderAll();
          _self.canvas.fire('object:property-realy-changed');
          return;
        }
      ...
      $(`${this.containerSelector} .toolpanel#select-panel #textStrokeWidth`).change(function () {
        const value = parseFloat($(this).val());
        if (!Number.isFinite(value) || value < 0 || !_self.activeSelection) return;
    ...
    (() => {
      $(`${this.containerSelector} .toolpanel#select-panel .content > .title`).after(`
        <div class="geometry-section">
          ...
          <div class="geometry-grid">
            ...
            <label><span>${window.lang == 'ru' ? 'Ширина' : 'Width'}</span><input type="number" min="1" step="1" data-geometry="width"></label>
            <label><span>${window.lang == 'ru' ? 'Высота' : 'Height'}</span><input type="number" min="1" step="1" data-geometry="height"></label>
      ...
      const updateGeometry = input => {
        ...
        if (property === 'x' || property === 'y') {
          ...
          object.setPositionByOrigin(new fabric.Point(
            property === 'x' ? value : center.x,
            property === 'y' ? value : center.y,
          ), 'center', 'center');
      ...
      $(`${this.containerSelector} .toolpanel#select-panel [data-geometry]`)
        .on('input', function () {
          if (this.value !== '') updateGeometry(this);
        })
        .on('change', function () {
    ...
    (() => {
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .object-options #delete`).click(() => {
        ...
        this.canvas.discardActiveObject().requestRenderAll(), this.canvas.fire('object:property-realy-changed');
      })
    ...
    this.setSelectionValues = () => {
      ...
      if (count == 1) {
        ...
        if (activeObject) {
          ...
          const setValue = (selector, value, silent = true) => {
            if (value !== undefined && value !== null) {
              $(`${_self.containerSelector} .toolpanel#select-panel ${selector}`).val(value)
            }
          };

          const center = activeObject.getCenterPoint();
          setValue('[data-geometry="x"]', Math.round(center.x));
          setValue('[data-geometry="y"]', Math.round(center.y));
          setValue('[data-geometry="width"]', Math.round(activeObject.getScaledWidth()));
          setValue('[data-geometry="height"]', Math.round(activeObject.getScaledHeight()));
          setValue('[data-geometry="angle"]', Math.round(activeObject.angle || 0));
          $(`${_self.containerSelector} .toolpanel#select-panel [data-geometry="angle"]`)
            .prop('disabled', activeObject.lockRotation === true);
          ...
          if (type === 'textbox' || type === 'i-text' || type === 'text') {
            $(`${_self.containerSelector} .toolpanel#select-panel .geometry-hint`).text(
              window.lang == 'ru'
                ? 'Здесь ширина и высота — масштаб. Переносы меняются полем «Ширина области текста».'
                : 'Width and height here are scale. Change wrapping with Text Box Width below.',
            );
            // Text section
            ...
            setValue('.text-section #textBoxWidth', Math.round(activeObject.width || 0));
