...
(function () {
  ...
  var selectionSettings = function () {
    ...
    const syncGeometryValues = (object, editedInput = null) => {
      ...
      const uniformScale = Math.abs(scaleX - scaleY) < 0.0005;

      setGeometryValue('x', Math.round(center.x));
      setGeometryValue('y', Math.round(center.y));
      ...
      setGeometryValue('scale', uniformScale ? roundedScaleX : '');
      setGeometryValue('angle', Math.round(object.angle || 0));

      const sizeSummary = document.querySelector(
        `${_self.containerSelector} .geometry-size-summary`,
      );
      if (sizeSummary) {
        sizeSummary.textContent = uniformScale
          ? `${window.lang == 'ru' ? 'Итоговый размер' : 'Final size'}: ${width} × ${height} px`
          : `${window.lang == 'ru' ? 'Итоговый размер' : 'Final size'}: ${width} × ${height} px · Scale X ${roundedScaleX}, Y ${roundedScaleY}`;
      }
    ...
    (() => {
        ...
        <div class="text-section">
          ...
          <div class="sizes">
            ...
            <div class="input-container">
              ...
              ${rangeNumberControl('textBoxWidth', 50, 10000, 10, 1000, 50, 4000)}
            </div>
      ...
      $(`${this.containerSelector} .toolpanel#select-panel #textBoxWidth`).on('input change', function (event) {
        const width = Number($(this).val());

        if (!validNumberInput(this, width)) {
          if (event.type === 'change') {
            _self.setSelectionValues();
          }
          return;
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .content > .title`).after(`
        <div class="geometry-section">
          ...
          <div class="geometry-grid">
            ...
            <label class="geometry-uniform-scale"><span>${window.lang == 'ru' ? 'Масштаб' : 'Scale'}</span><input type="number" min="0.01" max="100" step="0.01" placeholder="X/Y" data-geometry="scale"></label>
        ...
        if (activeObject) {
          ...
          if (type === 'textbox' || type === 'i-text' || type === 'text') {
            $(`${_self.containerSelector} .toolpanel#select-panel .geometry-hint`).text(
              window.lang == 'ru'
                ? 'Углы и «Масштаб» увеличивают весь текст. Боковые хендлы и «Ширина области текста» меняют переносы.'
                : 'Corners and Scale resize the whole text. Side handles and Text Box Width change wrapping.',
            );
    ...
    ['object:modified', 'object:property-realy-changed'].forEach(eventName => {
      _self.canvas.on(eventName, event => {
