...
(function () {
  ...
  var selectionSettings = function () {
    ...
    (() => {
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .color #color-picker`).change(function (_event, selectedColor) {
        const color = window.colorToHex(selectedColor || $(this).spectrum('get'));
        if (!color) return;
        this.value = color;
      ...
      $(`${this.containerSelector} .toolpanel#select-panel #text-stroke-color-picker`).change(function (_event, selectedColor) {
        if (!_self.activeSelection) return;
        const color = window.colorToHex(selectedColor || $(this).spectrum('get'));
        if (!color) return;
        this.value = color;
        _self.setActiveFontStyle(_self.activeSelection, 'stroke', color);
    ...
    (() => {
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .fill-section #color-picker`).spectrum({
        ...
        preferredFormat: 'hex8',
        move: function (color) {
          const hex = window.colorToHex(color);
          if (!hex) return;
          this.value = hex;
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .fill-section #color-picker`).change(function () {
        const color = window.colorToHex($(this).spectrum('get'));
      ...
      gp.addHandler(0, '#ff0000');
      gp.addHandler(100, '#0000ff');
      ...
      if (count == 1) {
        ...
        if (activeObject) {
          ...
          $(`${_self.containerSelector} .toolpanel#select-panel .fill-section #color-picker`)
            .spectrum('set', window.spectrumInputColor(fill || '#000000'));
          ...
          if (type === 'i-text') {
            ...
            $(`${_self.containerSelector} .toolpanel#select-panel #text-stroke-color-picker`)
              .spectrum('set', window.spectrumInputColor(stroke || '#000000'));
          }
          else {
            $(`${_self.containerSelector} .toolpanel#select-panel .geometry-hint`).text(
              window.lang == 'ru'
