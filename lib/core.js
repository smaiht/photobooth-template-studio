    ...
    this.setActiveTool = (id, forceClose = false) => {
      ...
      if (forceClose) {
        if (visiblePanel && visiblePanel.id === `${id}-panel`) {
          $(`${containerSelector} #toolbar button#${id}`).removeClass('active');
          this.activeTool = null;
          return;
        }
      }
      ...
      this.canvas.forEachObject(o => {
        o.selectable = o.editorLocked !== true;
        o.evented = o.editorLocked !== true;
      })
          ...
          <div class="content template-export-content">
            ...
            <section class="template-import-section">
              ...
              <p class="section-hint">
                ${window.lang == 'ru'
                  ? 'Выберите config.json и указанный в нём фон. Фото, подписи и обрезка восстановятся автоматически.'
                  : 'Choose config.json and its background. Photo slots, text and trim are restored automatically.'}
              </p>
              <label class="template-import-file">
                <span>config.json</span>
                <input id="import-template-config" type="file" accept=".json,application/json">
              </label>
              <div class="input-container" id="import-template-choice" hidden>
                <label>${window.lang == 'ru' ? 'Макет из конфига' : 'Layout from config'}</label>
                <select id="import-template-key"></select>
              </div>
              <label class="template-import-file">
                <span>${window.lang == 'ru' ? 'Фоновое изображение' : 'Background image'}</span>
                <input id="import-template-background" type="file" accept="image/jpeg,image/png,image/webp">
                <small id="import-template-background-hint"></small>
              </label>
              <div class="template-export-actions">
                <button class="primary" id="import-template-run" type="button" disabled>
                  ${window.lang == 'ru' ? 'Импортировать шаблон' : 'Import template'}
                </button>
            ...
            <section class="template-export-section">
              ...
              <p class="section-hint">
                ${window.lang == 'ru'
                  ? 'Конфиг, готовый фон и используемые шрифты.'
                  : 'Config, rendered background and used fonts.'}
              </p>
              <div class="input-container">
                <label>${window.lang == 'ru' ? 'Ключ шаблона' : 'Template key'}</label>
                <input id="export-template-key" type="text" value="grid" spellcheck="false">
              </div>
              <div class="input-container">
                <label>${window.lang == 'ru' ? 'Название' : 'Label'}</label>
                <input id="export-template-label" type="text" value="Открытка">
              </div>
