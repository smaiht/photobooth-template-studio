...
fabric.Canvas.prototype._historyNext = function () {
  ...
  historyData.printTrim = this.printTrim ? { ...this.printTrim } : null;
...
fabric.Canvas.prototype._loadHistory = function (history, event, callback) {
  const that = this;
  const projectRevision = that.projectRevision || 0;
  ...
  if (parsedHistory.objects) {
    fabric.util.enlivenObjects(parsedHistory.objects, (objects) => {
      if ((that.projectRevision || 0) !== projectRevision) {
      ...
      if (parsedHistory.printTrim) {
        that.printTrim = { ...parsedHistory.printTrim };
        that.fire('trim:history', { trim: that.printTrim });
      }
