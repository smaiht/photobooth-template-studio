...
(function () {
  ...
  function photoLayout() {
    ...
    `).join('');
    const clearanceSide = side => `
      <div class="photo-clearance-side photo-clearance-${side}">
        <span class="photo-clearance-edge">
          <i aria-hidden="true"></i>
          <output data-photo-distance="edge-${side}">—</output>
        </span>
        <span class="photo-clearance-trim">
          <i aria-hidden="true"></i>
          <output data-photo-distance="trim-${side}">—</output>
        </span>
      </div>
    `;
    const measurementCard = (side, title) => `
      <div class="photo-measurement-card" data-photo-measurement="${side}"${side === 'right' ? ' hidden' : ''}>
        ...
        <div class="photo-clearance-legend">
          <span class="photo-clearance-edge"><i aria-hidden="true"></i>${isRussian ? 'до края' : 'to edge'}</span>
          <span class="photo-clearance-trim"><i aria-hidden="true"></i>${isRussian ? 'до trim / cut' : 'to trim / cut'}</span>
        </div>
        <div class="photo-clearance-diagram">
          ${clearanceSide('top')}
          ${clearanceSide('left')}
          <div class="photo-clearance-canvas">
            <span class="photo-clearance-red-frame"></span>
            <span class="photo-clearance-photos">
              <small>${isRussian ? 'ФОТО' : 'PHOTOS'}</small>
              <output class="photo-layout-bounds-size" data-photo-bounds-size>—</output>
            </span>
          </div>
          ${clearanceSide('right')}
          ${clearanceSide('bottom')}
        </div>
    ...
    const renderMeasurements = () => {
      ...
      const renderSide = (side, sideGeometries, trim, cutSide = null) => {
        ...
        ui.trimReference.textContent = isRussian
          ? `Trim / cut: слева ${trimValue('left')} · сверху ${trimValue('top')} · справа ${trimValue('right')} · снизу ${trimValue('bottom')}`
          : `Trim / cut: left ${trimValue('left')} · top ${trimValue('top')} · right ${trimValue('right')} · bottom ${trimValue('bottom')}`;
    ...
    const setLayoutUnlocked = (unlocked, keepPanel = true) => {
      ...
      canvas.getObjects().forEach(object => {
        ...
        const interactive = !layoutUnlocked && object.editorLocked !== true;
