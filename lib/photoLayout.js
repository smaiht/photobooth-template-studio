...
(function () {
  ...
  function photoLayout() {
    ...
    const defaultGeometry = defaultPreset.slots[0];
    ...
    const presetButtons = config.presets.map(preset => `
      <button type="button" data-photo-preset="${escapeHTML(preset.id)}">
    ...
    const slotGeometry = slot => ({
      ...
      width: Math.max(MIN_SLOT_SIZE, Math.round(slot.getScaledWidth())),
      height: Math.max(MIN_SLOT_SIZE, Math.round(slot.getScaledHeight())),
    });
    ...
    const setSlotsLocked = () => {
      getSlots().forEach(slot => {
        const isSymmetryFollower = symmetryEnabled && slot.photoIndex !== 0;
        ...
        slot.editorLocked = !layoutUnlocked || !slotsVisible || isSymmetryFollower;
        slot.selectable = layoutUnlocked && slotsVisible && !isSymmetryFollower;
        slot.evented = layoutUnlocked && slotsVisible && !isSymmetryFollower;
        slot.setControlsVisibility({ mtr: false, deleteControl: false });
      });
    };
    ...
    const activePresetId = () => (
      config.presets.find(preset => presetMatches(preset))?.id || null
    );
    ...
    const renderSlotCards = () => {
      ...
      slotList.innerHTML = slots.map(slot => {
        ...
        const geometryEditable = layoutUnlocked
          && (!symmetryEnabled || slot.photoIndex === 0);
        ...
        return `
          <article class="photo-slot-card${activeObject === slot ? ' selected' : ''}${symmetryEnabled && slot.photoIndex !== 0 ? ' symmetry-follower' : ''}" data-photo-index="${slot.photoIndex}">
            <header>
    ...
    const applyGeometries = (
      ...
      if (nextSymmetry) {
        symmetryEnabled = nextSymmetry.enabled === true && geometries.length <= 4;
    ...
    const autoGrid = count => {
      const matchingPreset = config.presets.find(preset => {
        ...
        return preset.slots.length === count
    ...
    const applySlotCard = (card, changedProperty, recordHistory = false) => {
      ...
      if (symmetryEnabled && slot.photoIndex !== 0) {
        return false;
      }
    ...
    const replaceSlot = (slot, previewType, image = null, fileName = null) => {
      ...
      mutateLayout(() => {
        ...
        if (layoutUnlocked && (!symmetryEnabled || index === 0)) {
    ...
    _self.syncPhotoLayout = (ensureDefault = true) => {
      ...
      if (!slots.length && ensureDefault) {
        ...
        aspectRatioSelect.value = aspectRatioId;
        symmetryEnabled = slots.length <= 4
    ...
    [
      [symmetryOffsetXInput, 'x', canvasWidth / 2],
      [symmetryOffsetYInput, 'y', canvasHeight / 2],
    ].forEach(([input, axis, center]) => {
