  ...
  function zoom() {
    ...
    const workspaceRect = () => {
      ...
      const panel = document.querySelector(
        `${editor.containerSelector} .toolpanel.visible`,
      );
    ...
    const resizeCanvas = value => {
      ...
      const width = editor.canvas.originalW * nextZoom;
      const height = editor.canvas.originalH * nextZoom;
    ...
    this.refreshWorkspaceLayout = () => {
      resizeCanvas(editor.canvas.getZoom());
      applyPan();
    };
    ...
    const scheduleWorkspaceRefresh = () => {
      ...
      resizeFrame = requestAnimationFrame(() => editor.refreshWorkspaceLayout());
    };
    window.addEventListener('resize', scheduleWorkspaceRefresh);
    window.visualViewport?.addEventListener('resize', scheduleWorkspaceRefresh);
    new ResizeObserver(scheduleWorkspaceRefresh).observe(canvasHolder);
