...
(function () {
  ...
  function zoom() {
        ...
        if (!this.lastMoveTime || Date.now() - this.lastMoveTime > 16) { // 16 for ~60fps
          ...
          if (lastDist) {
            ...
            if (lastX && lastY) {
              ...
              canvasContainer.style.transform = matrix.toString();
            }
          }
    ...
    this.fitZoom = () => {
      ...
      const containerWidth = canvasHolder.clientWidth - horizontalPadding;
      const containerHeight = canvasHolder.clientHeight - verticalPadding;
      ...
      // Сбрасываем трансформацию контейнера
